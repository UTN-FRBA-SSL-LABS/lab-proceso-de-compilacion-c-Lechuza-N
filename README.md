...
> **R:** Porque todas las directivas se "expandieron" y contribuyeron a llenar gran parte del programa.i, que no están en el programa.c
...
> **R:** Porque en el preprocesamieno se ignoran los comentarios y la coincidencia buscada solo aparece en un comentrario
...
> **R:** El comando sin -DDEBUG no devuelve nada y el que lo tiene devuelve
```bash
printf("[DEBUG] %s\n", ("Iniciando main"));
```
...
> **R:** Esas líneas comunican el numero de línea y el archivo original del código, que en el caso de printf es "/usr/include/stdio.h"
...
> **R:** Estas instrucciones indican el inicio de una función y preparan el contexto para su correcto funcionamiento
...
> **R:** Porque en programa.o no se hay definido, pero se necesita. Esa función se define solo en matematica.o, por eso aparece con T
...
> **R:** Porque son funciones de la biblioteca dinámica del sistema y se cargan en tiempo de ejecución. El enlazador deja registrado su nombre para que el cargador dinámico los resuelva en tiempo de ejecución
...
> **R:** La diferencia entre una macro función y una función real es que la macro "desaparece" durante el preprocesamiento, reemplazada por la expresión
que se haya definido. Mientras, la función real nunca "desaparece" como tal. Además, la función real cuenta con verificación de tipos.
...
> **R:** La diferencia clave es que el tipo T indica que está definido en la sección text del archivo objeto, como código ejecutable. Mientras que el de
tipo D indica que es una variable global con valor inicial en la sección data.
...
> **R:**
```bash
linux-vdso.so.1 (0x000070cfc5c16000)
libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6 (0x000070cfc5800000)
/lib64/ld-linux-x86-64.so.2 (0x000070cfc5c18000)
```

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque gcc la incluye automáticamente.
